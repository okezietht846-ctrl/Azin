...
class UserController extends Controller
{
    ...
    public function newbalanceUpdate($id, Request $request)
{
    // Validate the request parameters
    $validator = Validator::make($request->all(), [
        'amount' => 'required|numeric',
        'type' => 'required|in:add,subtract',
        'wallet' => 'required|in:depositbonus,investmentbonus',
    ]);
    ...
    try {
        ...
        // Find the user and the admin user
        $user = User::findOrFail($id);
        $adminUser = \Auth::user();

        // Perform the add or subtract operation
        if ($type == 'add') {
            $user->increment($wallet == 'depositbonus' ? 'totaldeposit_bonus' : 'totalInvestment_bonus', $amount);
            ...
            $user->decrement($wallet == 'depositbonus' ? 'totaldeposit_bonus' : 'totalInvestment_bonus', $amount);
            Txn::new($amount, 0, $amount, 'system', 'Money subtracted in '.ucwords($wallet).' Wallet from System', TxnType::Subtract, TxnStatus::Success, null, null, $id, $adminUser->id, 'Admin');
        ...
        // Log the exception and notify with the error message
        \Log::error($e);
        notify()->error(__('Something went wrong: ') . $e->getMessage(), 'warning');
     ...
     public function balanceUpdate($id, Request $request)
{
    ...
    try {
        ...
        if ($type == 'add') {
            if ($wallet == 'main') {
                // Update the main balance and log as a Deposit
                $user->balance += $amount;
                $user->save();
                Txn::new($amount, 0, $amount, 'system', 'Money added to Main Wallet from System', TxnType::Deposit, TxnStatus::Success, null, null, $id, $adminUser->id, 'Admin');
            } else {
                // Update the profit balance and log as a Bonus
                $user->profit_balance += $amount;
                $user->save();
                Txn::new($amount, 0, $amount, 'system', 'Money added to Profit Wallet from System', TxnType::Bonus, TxnStatus::Success, null, null, $id, $adminUser->id, 'Admin');
            }
            ...
            if ($wallet == 'main') {
                // Subtract from the main balance and log as a Deposit
                $user->balance -= $amount;
                $user->save();
                Txn::new($amount, 0, $amount, 'system', 'Money subtracted from Main Wallet by System', TxnType::Deposit, TxnStatus::Success, null, null, $id, $adminUser->id, 'Admin');
            } else {
                // Subtract from the profit balance and log as a Bonus
                $user->profit_balance -= $amount;
                $user->save();
                Txn::new($amount, 0, $amount, 'system', 'Money subtracted from Profit Wallet by System', TxnType::Bonus, TxnStatus::Success, null, null, $id, $adminUser->id, 'Admin');
            }
